...
template <class T> inline void print(T t)
{
    std::cout << t << std::endl;
}
...
template <class T, size_t N> void print_array(const std::array<T, N> &arr)
{
    std::cout << " ( ";
...
template <int N, class T> void print_vec(const glm::vec<N, T, glm::qualifier::defaultp> &vec)
{
    std::cout << " ( ";
    for (int i = 0; i < N; i++)
    {
