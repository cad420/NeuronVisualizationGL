...
class RendererConfig
{
  public:
    ...
    std::string getLodFile() const
    {
        ...
        {
            std::cout << "Not provide lod_file in config file, use default lod_file(mouse_file_config.json)."
                      << std::endl;
    ...
    std::tuple<std::array<float, 3>, std::array<float, 3>, std::array<float, 3>, float, float, float> getCameraSetting()
        const
    {
        ...
        if (d || j.find("camera") == j.end())
        {
            ...
            return std::make_tuple(std::array<float, 3>{0.f, 0.f, 0.f}, std::array<float, 3>{0.f, 0.f, -1.f},
                                   std::array<float, 3>{0.f, 1.f, 0.f}, 20.f, 1.f, 512.f);
        }
        else
        {
