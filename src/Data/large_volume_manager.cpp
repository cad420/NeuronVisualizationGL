...
LargeVolumeManager::LargeVolumeManager(const char *lod_config_file) : stop(false)
{
...
LargeVolumeManager::LargeVolumeManager(const std::unordered_map<int, std::string> &lods) : stop(false)
{
    this->volume_reader = std::make_unique<LargeVolumeReader>(lods);
    initResource();
    startWorking();
}
LargeVolumeManager::LargeVolumeManager(const char *lod_config_file, CUcontext ctx) : stop(false)
{
    setCUDActx(ctx);
    this->volume_reader = std::make_unique<LargeVolumeReader>(lod_config_file);
    initResource();
        ...
        while (true)
        {
            ...
            for (int i = 0; i < workers.size(); i++)
            {
                ...
                if (!worker.isBusy())
                {
                    ...
                    jobs->AppendTask(
                        [&](int idx, BlockDesc block_desc) {
                            std::vector<std::vector<uint8_t>> packet;

                            volume_reader->read_packet(
                                block_desc.block_index[3],
                                {block_desc.block_index[0], block_desc.block_index[1], block_desc.block_index[2]},
                                packet);

                            auto &cu_mem = cu_mem_pool->getCUMem();

                            START_CPU_TIMER
                            workers[idx].uncompress((uint8_t *)cu_mem.data_ptr, cu_mem_pool->getBlockSizeBytes(),
                                                    packet);
                            END_CPU_TIMER
