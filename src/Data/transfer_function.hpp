    ...
    for (int sb = 0; sb < TF_DIM; sb++)
    {
        for (int sf = 0; sf <= sb; sf++)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                float opacity =
                    (transfer_func[int(s) * 4 + 3] * (1.0 - sFrac) + transfer_func[((int)s + offset) * 4 + 3] * sFrac) *
                    stepWidth;
                float temp = std::exp(-rgba[3]) * opacity;
                rgba[0] +=
                    (transfer_func[(int)s * 4 + 0] * (1.0 - sFrac) + transfer_func[(int(s) + offset) * 4 + 0] * sFrac) *
                    temp;
                rgba[1] +=
                    (transfer_func[(int)s * 4 + 1] * (1.0 - sFrac) + transfer_func[(int(s) + offset) * 4 + 1] * sFrac) *
                    temp;
                rgba[2] +=
                    (transfer_func[(int)s * 4 + 2] * (1.0 - sFrac) + transfer_func[(int(s) + offset) * 4 + 2] * sFrac) *
                    temp;
                rgba[3] += opacity;
            }
            preint_transfer_func[(sf * TF_DIM + sb) * 4 + 0] = preint_transfer_func[(sb * TF_DIM + sf) * 4 + 0] =
                rgba[0];
            preint_transfer_func[(sf * TF_DIM + sb) * 4 + 1] = preint_transfer_func[(sb * TF_DIM + sf) * 4 + 1] =
                rgba[1];
            preint_transfer_func[(sf * TF_DIM + sb) * 4 + 2] = preint_transfer_func[(sb * TF_DIM + sf) * 4 + 2] =
                rgba[2];
